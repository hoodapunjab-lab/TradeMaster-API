...
class TradeBrain:
    ...
    def train_brain(self):
        ...
        for sym in stock_list:
            df = db_engine.fetch_data(sym, "15m", limit=2000)
            if not df.empty and len(df) > 100:
                df = self.engineer_features(df)
                # ✅ DHOKEBAJI FIX: 0.2% Minimum Move required to classify as BUY
        ...
        features = ['return', 'log_return', 'rel_vol', 'dist_vwap', 'atr_proxy', 'dist_ma', 'rsi', 'macd_diff', 'bb_width', 'bb_pos']
        ...
        self.xgb_model = xgb.XGBClassifier(n_estimators=100, learning_rate=0.05, max_depth=5, n_jobs=-1, random_state=42)
        self.xgb_model.fit(X_scaled, y)
        joblib.dump({'model': self.xgb_model, 'features': self.features_list}, MODEL_XGB)

        print("⚡ Training LightGBM...")
        self.lgbm_model = lgb.LGBMClassifier(n_estimators=100, learning_rate=0.05, num_leaves=31, n_jobs=-1, random_state=42, verbose=-1)
    ...
    def predict_signal(self, symbol):
        ...
        try:
            df = db_engine.fetch_data(symbol, "15m", limit=100)
            ...
            prob_xgb = float(self.xgb_model.predict_proba(last_row_scaled)[0][1])
            
            prob_lgbm = 0.5
            if self.lgbm_model:
                prob_lgbm = float(self.lgbm_model.predict_proba(last_row_scaled)[0][1])

            prob_lstm = 0.5
            if self.lstm_model and TF_AVAILABLE:
                try:
                    X_input = last_row_scaled.reshape((1, 1, last_row_scaled.shape[1]))
                    prob_lstm = float(self.lstm_model.predict(X_input, verbose=0)[0][0])
                except: pass

            # Ensemble Logic
            final_score = (prob_xgb * 0.4) + (prob_lgbm * 0.4) + (prob_lstm * 0.2)
            confidence = round(final_score * 100, 2)
            
            signal = "NEUTRAL"
            if confidence >= 70: signal = "BUY"
            elif confidence <= 30: signal = "SELL"
            
            return {"signal": signal, "prob": float(confidence)}
