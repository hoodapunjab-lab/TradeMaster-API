...
# Yahan hum market ka aakhri mood save karenge
...
class MarketScanner:
    ...
    # ✅ FIXED: Default timeframe set to "5m"
    def analyze_stock(self, symbol, timeframe="5m"):
        ...
        try:
            ...
            regime_score = 0
            if REGIME_AVAILABLE:
                # ✅ FIXED CRASH BUG: Added 'symbol' argument to get_regime
                stock_regime = regime_detector.get_regime(symbol, df)
                if "CHOPPY" in stock_regime: regime_score -= 20
                if "TRENDING UP" in stock_regime: regime_score += 10
                if "TRENDING DOWN" in stock_regime: regime_score -= 10 # ✅ Added Shorting logic
                
                stock_sector = symbols.get_sector(symbol)
                if stock_sector:
                    sector_df = db_engine.fetch_data(stock_sector, timeframe, limit=100)
                    if not sector_df.empty:
                        # ✅ FIXED CRASH BUG: Added 'stock_sector' argument
                        sector_regime = regime_detector.get_regime(stock_sector, sector_df)
            ...
            # ✅ THE ULTIMATE FIX: Strictly cap score between 0 and 100
            # This prevents UI bugs while maintaining accurate Sell logic
            score = max(0, min(int(score), 100))
            ai_score = min(100.0, float(ai_score))
            ...
            # ✅ FIXED REVERSE SL BUG: Dynamic Stop Loss & Target for Buy and Sell
            if signal == "SELL":
                sl = current_price + (2 * atr)  # Short mein SL upar hota hai
                target = current_price - (4 * atr) # Target neeche hota hai
            else:
    ...
    # ✅ FIXED: Default timeframe set to "5m"
    def scan_market(self, timeframe="5m"):
        results = []
