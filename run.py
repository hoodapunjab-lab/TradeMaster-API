...
def start_system():
    ...
    try:
        while True:
            ...
            # ✅ FIXED Watchdog: BOT Auto-Restart & Alert
            if processes['bot'].poll() is not None:
                print(f"{RED}🚨 BOT CRASHED! Restarting immediately to protect open trades...{RESET}")
                send_alert("🚨 WARNING: Order Manager crashed! Auto-restarting immediately to protect open trades.")
                # Bot ko turant wapas zinda karna zaroori hai
                processes['bot'] = subprocess.Popen([sys.executable, "order_manager.py"])
                time.sleep(2) # Thoda sa buffer time naye instance ko set hone ke liye
