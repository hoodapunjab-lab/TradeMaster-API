...
def fetch_data_smart(symbol, timeframe="15m"):
    """
    ✅ 100% ANGEL ONE DATABASE FETCH
    """
    # 1. Angel One / JSON ke hisaab se symbol set karein
    ...
    try:
        # 2. Local Database se sidha latest price fetch karein (500 candles for AI)
        df = db_engine.fetch_data(db_sym, timeframe, limit=500)
        
        # Fallback agar bina space ke save hua ho
        if df.empty:
...
def analyze_option_chain(symbol):
    ...
    # ✅ FIX: Auto-add new symbols (like NIFTY IT, AUTO) to Cache Memory dynamically
    ...
    try:
        df = fetch_data_smart(symbol, "15m")
        ...
        try:
            ...
            adx = 20; rsi = 50; atr = current_price * 0.01; ema_20 = current_price
        ...
        if is_market_open() and ("CE" in signal or "PE" in signal):
            if (current_time_epoch - cache_data[symbol]["last_alert_time"]) > 900: 
        ...
        # traceback.print_exc() # Muted for clean console
